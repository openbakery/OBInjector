...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    private func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
		...
		OBInjectorController.configureInjector({ (injector:OBPropertyInjector!) -> Void in
			let service = MyService()
			injector.registerProperty("myService", withInstance:service)
			
			
            injector.registerProperty("currentDate", with:{
				return NSDate();
			});
        ...
        OBInjectorController.injectDependencies(to: navigationController.topViewController)
